  ...
  <div class="full-modal" id="full-modal" :class="[$parent.loader || $parent.mute ? 'overflow-hidden' : 'overflow-auto']">
        <div class="flex min-h-full w-full justify-end relative" >
            ...
            <div id="modal" class="modal relative w-full lg:w-3/5 xl:1/2 bg-gray-300 shadow-2xl pb-16">
                <div class="absolute w-full w-full h-screen dark z-50 flex items-center justify-center" v-show="$parent.loader">
                ...
                <div class="fixed py-5 px-8  top-0 text-xl hover:text-red-500 text-gray-300 cursor-pointer z-50 dark transition-all easy-in-out" @click="closeModal">
                    <i class="fas fa-times"></i>
                </div>
                <div class="dark relative flex items-center justify-center h-[300px] md:h-[400px] px-5">
                    <h4 class="text-8xl md:text-9xl bebas text-darker text-center mb-4">{{$parent.projets[$parent.indexModal].title}}</h4>
                </div>
                 <div class="md:-mt-[180px] -mt-[130px] relative">
                    <div class="rounded-t-lg bg-gray-400 p-3 w-8/12 md:w-6/12 mx-auto transition-all easy-in-out duration-900 bg-gray-400 ">
                        <img :src="'/img/projets/' + $parent.projets[$parent.indexModal].image[0]" alt="" class="w-full z-0"/>
                    </div>
                    <div class="rounded-b-lg w-9/12 md:w-7/12 h-4 bg-gray-500 group-hover:bg-green-500 mt-1 mx-auto transition-all easy-in-out duration-900 bg-gray-500"></div>
                </div>
                <div class="flex flex-wrap weight-regular text-black text-left mt-12">
                    <div class="w-full p-1 text-center">
                        <h4 class="text-4xl md:text-4xl font-bold text-darker text-center mb-1">{{$parent.projets[$parent.indexModal].title}}</h4>
                        ...
                        <div class="flex flex-wrap text-md uppercase justify-center mb-12">
                            <div v-for="(label, index) in $parent.projets[$parent.indexModal].labels" :key="index" class="p-1 text-black border border-black mr-2 px-2 mt-2">
                                {{label}}
                            </div>
                        </div>
                       <p class="text-md text-center lg:text-xl px-4 md:px-24 mt-10">{{ $parent.projets[$parent.indexModal].description }}</p>
                    </div>
                </div>
                <div class="flex flex-wrap justify-center items-center mt-16">
                    <a v-if="$parent.projets[$parent.indexModal].link" :href="$parent.projets[$parent.indexModal].link" class="">
                        <mainButton text="visiter le site" color="darker" colorText="text-gray-300 hover:text-black"></mainButton>
                    </a>
                    <div class="ml-4 text-darker">
